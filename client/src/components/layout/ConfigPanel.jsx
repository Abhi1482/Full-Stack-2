...
const ConfigPanel = ({ component, onUpdate, onDelete, onClose }) => {
    ...
    const handleFilesChange = async (newFiles) => {
        ...
        const filesToUpload = newFiles.filter(f => f.originFile);
        const existingFiles = newFiles.filter(f => !f.originFile);
        ...
        try {
            ...
            if (filesToUpload.length > 0) {
                // Upload new files
                const rawFiles = filesToUpload.map(f => f.originFile);
                const response = await fileAPI.upload(component.id, rawFiles);

                // Normalize uploaded files (ensure id property)
                uploadedFiles = response.data.map(f => ({
                    ...f,
                    id: f._id || f.id,
                    name: f.originalName || f.name, // Ensure name property matches UI expectation
                    size: f.size,
            ...
            // Update component metadata references
            onUpdate(component.id, {
                title,
                metadata: {
                    ...component.metadata,
            ...
            // Revert to existing files if upload fails
